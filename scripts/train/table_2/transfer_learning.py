...
PATIENCE = 1
memmaps_description = {}
EXPERIMENT_OUTPUT_FOLDER = EXPERIMENTS_DIRECTORY + "/table_2_DT/"
settings = {
    "mass": MASS_SETTINGS,
    "dodo": DODO_SETTINGS,
    "dodh": DODH_SETTINGS,
...
datasets_split = {}
datasets = ["sleep_edf", "dodh",'dodo','mesa','mros','shhs','mass','cap']
max_records_per_dataset = [128]
    ...
    for i in range(len(datasets)):
        ...
        for dataset_name in datasets:
            ...
            if dataset_name != target_dataset:
                record_to_sample_from_dataset[dataset_name] = min(record_per_dataset[dataset_name],n_records)
        ...
        while sampled_records < records_to_sample:
            ...
            for dataset_name in datasets:
                if dataset_name != target_dataset:
                    if record_to_sample_from_dataset[dataset_name] < record_per_dataset[dataset_name] and sampled_records < records_to_sample:
        ...
        for dataset_name in datasets:
            ...
            if dataset_name != target_dataset:
                records_for_train_and_validation = list(df.index[:record_to_sample_from_dataset[dataset_name]])
