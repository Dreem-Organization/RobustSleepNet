"""
Code to train RobustSleepNet on hypnogram from a specific scorer on DODO or DODH and to evaluate on the remaining 7 datasets.
"""
...
from scripts.train.table_3.add_scorers_hypnogram import add_scorers_hypnograms
...
PATIENCE = 5
memmaps_description = {}
EXPERIMENT_OUTPUT_FOLDER = EXPERIMENTS_DIRECTORY + "/scorers_transfer_train/"
CROSS_EXPERIMENT_OUTPUT_FOLDER = EXPERIMENTS_DIRECTORY + "/scorers_transfer_eval/"
settings = {
    "mass": MASS_SETTINGS,
    "dodo": DODO_SETTINGS,
    "dodh": DODH_SETTINGS,
...
scorers = ['scorer_1','scorer_2','scorer_3','scorer_4','scorer_5']
possible_target_datasets = ['mass','sleep_edf','mros','mesa','shhs','cap']
datasets = source_datasets + possible_target_datasets
...
for i in range(len(source_datasets)):
    ...
    target_datasets = [dataset for dataset in possible_target_datasets if dataset != source_dataset]
    ...
    for dataset_name in datasets:
        ...
        if not force:
            ...
            try:
                ...
                records_in_dataset = list(records_in_dataset.keys())
                records_in_dataset.sort()
                records_in_dataset = [f'{setting["h5_directory"]}/{x}' for x in records_in_dataset]
        ...
        if dataset_name == source_dataset:
            ...
            for scorer in scorers:
                add_scorers_hypnograms(memmaps_directory,dataset_name,scorer)
        ...
        for dataset in target_datasets:
            if dataset != source_dataset:
                ...
                experiment_to_log = {
                    "memmap_description": memmaps_description[source_dataset],
                    "dataset_settings": settings[source_dataset],
                    "trainer_parameters": trainer_eval,
                    "normalization_parameters": model_normalization,
                    "net_parameters": model_description,
                    "dataset_parameters": {
                        "temporal_context": TEMPORAL_CONTEXT,
