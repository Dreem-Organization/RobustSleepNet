...
from .epochs_reduction import epoch_reducers
...
class ModuloNet(nn.Module):
    ...
    def __init__(self, groups, features, normalization_parameters, net_parameters):
        ...
        if isinstance(normalization_parameters["signals"], list) and isinstance(
            normalization_parameters["features"], list
        ):
        ...
        self.groups_encoder, self.groups_reducer = {}, {}
    ...
    def init_net(self):
        ...
        encoders_input_shape, features_shape = {}, {}
        if len(self.groups) > 0:
            for group, value in self.groups.items():
                value = copy.deepcopy(value)
                value["shape"][0] *= self.net_parameters["input_temporal_context"]
                shape = (1, 1) + tuple(value["shape"][1:]) + (value["shape"][0],)
                encoders_input_shape[group] = np.random.normal(size=shape)
                encoders_input_shape[group] = (
                    torch.from_numpy(encoders_input_shape[group]).float().to(self.device)
                )
            encoders_input_shape = normalize(
                encoders_input_shape, self.normalization_parameters["signals"]
            )
            ...
            for group in self.groups:
                ...
                reducer_type = self.net_parameters["reducers"][group]["type"]
                reducer_args = self.net_parameters["reducers"][group]["args"]

                self.groups_encoder[group] = epoch_encoders[encoder_type](
                    self.groups[group], encoder_args
                )
                ...
                self.groups_reducer[group] = epoch_reducers[reducer_type](
                    self.groups[group], **reducer_args
                ).to(encoders_input_shape[group].device)
                features_shape[group] = self.groups_reducer[group](
                    encoders_input_shape[group]
                ).size()[-1]

                for key, module in self.groups_encoder.items():  # register blocks
                    self.add_module(f"encoder_{key}", module)
                for key, module in self.groups_reducer.items():  # register blocks
                    self.add_module(f"reducer_{key}", module)

        else:
            self.groups_encoder = None
        if len(self.features) > 0 and "features_encoder" in self.net_parameters:
            ...
            self.features_encoder = features_encoders[feature_encoder_type](
                self.features, **feature_encoder_args
            )
        else:
            if len(self.features) > 0:
                warnings.warn(
                    "The dataset contains features but no features encoder are "
                    "provided. Features will be ignored."
                )
            self.features_encoder = None

        if self.features_encoder is not None:
            assert "expert_features" not in features_shape
            features_shape["expert_features"] = self.features_encoder.out_features
        ...
        sequence_encoder_input_size = 0
        for feature_size in features_shape.values():
            sequence_encoder_input_size += feature_size
        self.sequence_encoder = sequence_encoder_cls(
            sequence_encoder_input_size, **sequence_encoder_params
        )
        self.classifier = nn.Linear(
            self.sequence_encoder.output_size, self.net_parameters["n_class"]
        )
    ...
    def forward_features(self, x):
        ...
        """
        features = {}
        for group, signals in x.items():
            batch_size, temporal_context = signals.size()[:2]
            features[group] = self.groups_encoder[group](signals)

        for group, feature in features.items():
            features[group] = self.groups_reducer[group](feature)

        # features = features.view((batch_size, temporal_context, -1))
        return features, batch_size, temporal_context
    ...
    def forward(self, x, sequence_hidden_state=None):
        ...
        if self.groups_encoder is not None:
            ...
            features, batch_size, temporal_context = self.forward_features(signals)

        if self.features_encoder is not None:
            epoch_features = normalize_features(
                x["features"], self.normalization_parameters["features"]
            )
            epoch_features = self.features_encoder.forward(epoch_features)
            features["expert_features"] = epoch_features
            batch_size, temporal_context = epoch_features.size()[:2]
            features["expert_features"] = features["expert_features"].reshape(
                batch_size * temporal_context, -1
            )

        features = torch.cat(list(features.values()), -1)
        features = features.view((batch_size, temporal_context, -1))
    ...
    def get_args(self, data):
        ...
        args["signals"] = {
            group: data["groups"][group].to(self.device, non_blocking=True) for group in self.groups
        }
        if self.features_encoder is not None:
            args["features"] = {
                feature: data["features"][feature].to(self.device, non_blocking=True)
    ...
    def predict_on_record(
        ...
        for batch, idxs in dataset.get_record(
            record,
            mode="eval",
            stride=stride,
            return_index=True,
            batch_size=batch_size,
            modality_to_use=modality_to_use,
        ):
            ...
            try:
                preds, hidden_state = (
                    ...
                    [pred.cpu().detach().numpy() for pred in preds[1]],
                )
            except AttributeError:
                preds, hidden_state = (
                    preds[0].cpu().detach().numpy(),
                    [h[0].cpu().detach().numpy() for h in preds[1]],
                )
            ...
            for i in range(batch_size):
                if output_mode == "many":
                    predicted_output[
                        idxs[i] - temporal_context // 2 : idxs[i] + temporal_context // 2 + 1
                    ] += preds[i] / temporal_context
    ...
    def predict_on_dataset(
        ...
        for record in enumerator:
            result[record] = self.predict_on_record(
                record,
                dataset,
                stride,
                return_prob,
        ...
        with tarfile.open(filename, "r") as tar:
            ...
            for var_type in ["signals", "features"]:
                for i, group in enumerate(normalization_parameters[var_type]):
                    for operation in group["normalization"]:
                        if (
                            operation["type"] == "standardize"
                            or operation["type"] == "standardization"
                        ):
                            operation["args"]["mu"] = torch.Tensor(operation["args"]["mu"])
                            operation["args"]["sigma"] = torch.Tensor(operation["args"]["sigma"])

            groups = json.loads(tar.extractfile("groups.json").read().decode("utf-8"))

            features = json.loads(tar.extractfile("features.json").read().decode("utf-8"))
