...
def cyclic_index_window(
    ...
    for cycle_length in cycles_length:
        result += [
            np.cos(
                np.pi
                * np.expand_dims(np.concatenate([padding_pre, index_window, padding_post]), -1)
                / cycle_length
            )
        ]
...
def spectral_power(
    ...
    nperseg, noverlap, nperepoch = (
        int(stft_duration * fs),
        int(stft_overlap * fs),
        int(epoch_duration * fs),
    )
...
def signal_statistics(
    signal, signal_properties, statistics, epoch_duration=30, window_duration=5, padding_duration=0,
):
    fs = signal_properties["fs"]
