...
def get_sleep_stages(annotation_file):
    ...
    with pyedflib.EdfReader(annotation_file) as annotation_file:
        ...
        time_begin, time_end = (
            annotations[0][stage_idx[0]],
            annotations[0][stage_idx[-1]] + annotations[1][stage_idx[-1]],
        )
                    ...
                    if mod_fs is not None:
                        ...
                        if (
                            ...
                            if crop_record:
                                ...
                                h5_target.create_dataset(
                                    signal_path,
                                    data=data.readSignal(signal_idx),
                                    compression="gzip",
                                )
                            signal_description = {
                                "fs": mod_fs,
            ...
            for annotation_file in annotation_files:
                with pyedflib.EdfReader(annotation_file) as annotations:
                    for name, properties in SS3_events.items():
                        result, time_begins, durations = get_annotation(
                            annotations, name, properties["fs"]
                        )
                        ...
                        if crop_record:
                            ...
                            h5_target.create_dataset(
                                event_path + "binary",
                                data=result[begin_idx:end_idx],
                                compression="gzip",
                                dtype="i8",
                            )
                        else:
                            h5_target.create_dataset(
                                event_path + "binary", data=result, compression="gzip", dtype="i8",
                            )
                        h5_target.create_dataset(
                            event_path + "begin", data=time_begins, compression="gzip"
                        )
                        h5_target.create_dataset(
                            event_path + "duration", data=durations, compression="gzip"
                        )
                        event_description = {
                            "name": event_name,
                            "fs": properties["fs"],
...
if __name__ == "__main__":
    ...
    from scripts.settings import MASS_SETTINGS
    from joblib import Parallel, delayed

    records_directory, annotations_directory, h5_directory = (
        MASS_SETTINGS["records_directory"],
        MASS_SETTINGS["annotations_directory"],
        MASS_SETTINGS["h5_directory"],
    )
    ...
    def record_to_h5_full(record, annotation):
        ...
        to_h5(
            records_directory + record,
            [annotations_directory + annotation],
            output_file,
            signals=signals,
        )
    ...
    if parallel is True:
        Parallel(n_jobs=-1)(
            delayed(record_to_h5_full)(records[i], annotations[i]) for i in range(len(records))
        )
    else:
