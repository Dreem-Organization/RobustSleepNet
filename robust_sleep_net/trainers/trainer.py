...
def collate_with_various_number_of_channels(data):
    collated_data = {"groups": {}, "features": {}, "hypnogram": {}}
    collated_data["hypnogram"] = torch.stack([x["hypnogram"] for x in data], 0)
    available_features = list(data[0]["features"].keys())
    available_groups = list(data[0]["groups"].keys())

    for group in available_groups:
        group_shape = data[0]["groups"][group].shape[1]
        p = 1 / (np.arange(1, group_shape))
        p = p / np.sum(p)
        channels_to_sample = np.random.choice(
            np.arange(1, group_shape), size=np.random.randint(1, group_shape), replace=False,
        )

        collated_data["groups"][group] = torch.stack(
            [x["groups"][group][:, channels_to_sample] for x in data], 0
        )
    for feature in available_features:
        collated_data["features"][group] = torch.stack([x["features"][feature] for x in data], 0)
...
class Trainer:
    def __init__(
        self,
        net: ModuloNet,
        metrics=["cohen_kappa", "f1", "accuracy", "f1_macro"],
        epochs=30,
        metric_to_maximize="f1_macro",
        patience=None,
        batch_size=32,
        save_folder=None,
        loss=None,
        ...
        self.metrics = {
            ...
            for score, score_function in score_functions.items()
    ...
    def reset_optimizer(self):
        self.base_optimizer = optimizers[self.optimizer_params["type"]](
            self.net.parameters(), **self.optimizer_params["args"]
        )
    ...
    def validate(
        self,
        validation_dataset,
        return_metrics_per_records=False,
        verbose=False,
        ...
        metrics_epoch = {metric: [] for metric in self.metrics.keys()}
        metrics_per_records = {}
        hypnograms = {}
        predictions = self.net.predict_on_dataset(
            validation_dataset,
            return_prob=False,
            verbose=verbose,
    ...
    def validate_on_batch(self, data, mask=-1):
        ...
        return output.detach(), loss_validation.detach(), hypnogram.detach()
    ...
    def train(self, train_dataset, validation_dataset, verbose=1, reset_optimizer=True):
        ...
        metrics_final = {metric: 0 for metric in self.metrics.keys()}

        best_value = 0
        counter_patience = 0
        target_frequency = {
            k: 1 / np.sqrt(v) for k, v in train_dataset.target_frequency.items() if k >= 0
        }
        target_frequency = {
            k: v / np.sum(list(target_frequency.values())) for k, v in target_frequency.items()
        }
        min_weight = np.mean(list(target_frequency.values()))
        for i in [0,1,2,3,4]:
            if i not in target_frequency:
                target_frequency[i] = min_weight

        if (
            "inverse_frequency_weighting" in self.loss_args["args"]
            and self.loss_args["args"]["inverse_frequency_weighting"]
        ):
            args = {
                k: v
                for k, v in self.loss_args["args"].items()
                if k != "inverse_frequency_weighting"
            }
            args.update({"weight": target_frequency})
            print('Using inverse frequency weighting')
            print(target_frequency)
        else:
            args = self.loss_args["args"]
        self.loss_function = loss_functions[self.loss_args["type"]](**args)
        ...
        dataloader_train = DataLoader(
            ...
            collate_fn=collate_with_various_number_of_channels,
        )
        ...
        for epoch in range(0, self.epochs):
            ...
            for i, data in enumerate(t):
                self.on_batch_start()
                # dataloader_train.dataset.change_number_of_channels_to_sample_per_group()
                if verbose > 0:
                    if (i + 1) % update_postfix_every == 0 and i != 0:
                        ...
                        t.set_postfix(
                            ...
                            **{k: v / counter_update_postfix for k, v in running_metrics.items()}
                        )
            ...
            metrics_epoch, value = self.validate(
                validation_dataset=validation_dataset, stride=validation_dataset.temporal_context,
            )
            ...
            if self.save_folder:
                ...
                json.dump(
                    metrics_epoch, open(self.save_folder + str(epoch) + "_metrics_epoch.json", "w"),
                )

            if value > best_value:
                print("New best {} !".format(self.metric_to_maximize), value)
                # best_net = copy.deepcopy(self.net)
                metrics_final = {metric: metrics_epoch[metric] for metric in self.metrics.keys()}
                best_value = value
                counter_patience = 0
                if self.save_folder:
                    self.save_weights("best_net")
                    json.dump(
                        metrics_epoch, open(self.save_folder + "metrics_best_epoch.json", "w"),
                    )
            else:
                counter_patience += 1
