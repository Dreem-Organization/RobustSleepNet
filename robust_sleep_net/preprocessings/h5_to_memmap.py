    ...
    for signal in group_description["signals"]:
        if isinstance(signal, dict):
            ...
            if isinstance(signal, list):
                signal_found = False
                for candidate_signal in signal:
                    if candidate_signal in h5:
            ...
            if truncate_to_second:
                sig_duration = (
                    len(signals[-1]) // signals_properties[-1]["fs"]
...
def compute_epoch_features(h5, features_description, signals):
    ...
    for signal in signals:
        ...
        signals_properties = {
            "fs": record_description[signal]["fs"],
            "padding": 0,
...
def h5_to_memmaps(
    ...
    num_workers=0,
    force=False,
    truncate_to_second=False,
    error_tolerant=False,
    remove_hypnogram=False,
):
    ...
    def process_record(record):
        try:
            ...
            if not os.path.exists(save_directory):  # create it
                ...
                elif len(os.listdir(save_directory)) < 6:  # corrupted or not finished
                    shutil.rmtree(save_directory)
                    os.makedirs(save_directory + "signals")
                    os.makedirs(save_directory + "features")
            ...
            if not os.path.isfile(save_directory + "properties.json"):
                with h5py.File(record, "r") if isinstance(record, str) else record as h5:
                    ...
                    for group_description in memmap_description["signals"]:
                        ...
                        groups[group_name] = {}
                        (
                            groups["signals"][group_name],
                            groups["properties"][group_name],
                        ) = processs_group(
                    ...
                    for feature in memmap_description["features"]:
                        computed_features += [
                            {
                                "name": feature["name"],
                                "value": compute_epoch_features(
                                    h5, feature["processing"], feature["signals"]
                                ),
                            }
                        ]
                    for feature in computed_features:
                        if signals_duration is not None:
                            assert feature["value"].shape[0] == signals_duration // 30, (
                                "Invalid features_data shape for " + feature["name"]
                            )
                    ...
                    if "hypnogram" in h5 and remove_hypnogram is False:
                        ...
                        hypnogram = np.array([0] * truncated_duration)
                    ...
                    for group_name, group in groups["signals"].items():
                        if hypnogra_provided:
                            group_arr = np.memmap(
                                save_directory + "signals/" + group_name + ".mm",
                                dtype="float32",
                                mode="w+",
                                shape=group.shape,
                            )
                    ...
                    with open(save_directory + "signals_name.json", "w") as f:

                        out = {
                            x["name"]: (
                                x["signals_name"]
                                if "signals_name" in x
                                else {
                                    "signal_" + str(i): str(i) for i, _ in enumerate(x["signals"])
                                }
                            )
                            for x in memmap_description["signals"]
                        }
    ...
    if num_workers > 1:
        ...
        Parallel(n_jobs=num_workers, verbose=10)(
            delayed(process_record)(record) for record in records
        )
    else:
    ...
    return (
        memmap_directory + pipeline_hash + "/",
        groups_description[0],
        features_description,
    )
