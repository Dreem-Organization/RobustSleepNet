...
def standardize_online(x, eps=1e-15, axis=[1, -1]):
    # batch_size, temporal_context, n_channels, frequency_features, signal_length = x.shape
...
normalizers = {
    ...
    "standardize_online": standardize_online,
    "spectrogram": spectrogram,
    "clip_and_scale": clip_and_scale,
}

features_normalizer = {"clip_and_scale": clip_and_scale}
...
def initialize_standardization_parameters(dataset, normalization_parameters, blacklist=[]):
    ...
    records = set(dataset.records).difference(set(blacklist))
    for record in tqdm.tqdm(records):

        for sample in dataset.get_record(record, 32, mode="eval"):
            for var_type, var_name in vars.items():
                for group_name in sample[var_name]:
                    sample[var_name][group_name] = sample[var_name][group_name].to(
                        device, non_blocking=True
                    )
                for group_normalization in normalization_parameters[var_type]:

                    group_name = group_normalization["name"]
                    if group_name in sample[var_name]:
                        for normalization in group_normalization["normalization"]:
                            ...
                            if normalize_group:
                                ...
                                if normalization["n_updates"] == 0:
                                    ...
                                    normalization["args"]["mu"] = normalization["args"][
                                        "mu"
                                    ] * N / (N + 1) + 1 / (N + 1) * multi_mean(
                                        sample[var_name][group_name], axis=axis
                                    )
                                    normalization["args"]["sigma"] = normalization["args"][
                                        "sigma"
                                    ] * N / (N + 1) + 1 / (N + 1) * multi_std(
                                        sample[var_name][group_name], axis=axis
                                    )
                                ...
                                sample[var_name][group_name] = normalizers[normalization["type"]](
                                    sample[var_name][group_name], **normalization["args"]
                                )
